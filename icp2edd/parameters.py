...
def _check_param_attributes_convert(dict_=None):
    ...
    if dict_ is None:
        ...
        if not isinstance(dict_,dict):
            _logger.exception(f"Invalid convert type -{dict_}-. convert must be a dictionary."
...
def _check_param_attributes_sep(str_=None):
    ...
    if str_ is None:
        ...
        if not isinstance(str_, str):
            _logger.exception(f"Invalid separator type -{str_}-. Separator must be a single character."
                              f"Check {setupcfg.extraParam}.")
            raise
        elif len(str_) != 1:
            _logger.exception(f"Invalid separator length -{str_}-. Separator must be a single character."
                              f"Check {setupcfg.extraParam}.")
            raise
        else:
            return str_
