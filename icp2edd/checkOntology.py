...
def _check_property(icp_, edd_):
    ...
    for k, v in edd_.isSubPropertyOf.items():
        if k not in icp_.isSubPropertyOf.keys():
            _miss += 1
            _logger.warning(f'missing property -{k}- in ICOS CP')
...
def _check_class_property(icp_, edd_):
    ...
    for k, v in icp_.classHasProperty.items():
        if k in edd_.classHasProperty.keys():
            edd_prop_list = edd_.classHasProperty[k]
            for icp_prop in sorted(v):
                if icp_prop not in edd_prop_list:
                    if 'cpmeta' not in icp_prop:
                        _logger.warning(f"property of class -{k}- differ.\n"
                                        f"\tICOS CP -{icp_prop}- not found in icp2edd")
                    else:
                        _status = 1
                        _logger.error(f'property of class -{k}- differ.\n'
                                      f'\tICOS CP -{icp_prop}- not found in icp2edd')
        else:
            _miss += 1
            if 'cpmeta' not in k:
    ...
    if _miss:
        _logger.warning(f'{_miss} ICOS CP properties are missing in icp2edd classes')
